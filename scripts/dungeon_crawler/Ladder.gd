...
func on_entity_move_into(entity: GridEntity, from_position: Vector3, new_pos: Vector3) -> bool:
	...
	var moving_dot = dir.normalized().dot(self.basis.z.normalized())
	if moving_dot > -0.9:
	...
	var facing_dot = entity.basis.z.dot(self.basis.z.normalized())
	if facing_dot > -0.9:
		entity.target_rotation = self.global_basis.get_rotation_quaternion()
		await get_tree().create_timer(0.1).timeout

	entity.target_position = from_position - self.basis.z.normalized() * entity.GRID_SIZE / 3
	await get_tree().create_timer(.2).timeout
	entity.target_position = from_position + Vector3(0, entity.GRID_SIZE, 0)  - self.basis.z.normalized() * entity.GRID_SIZE / 2.5
...
func on_entity_move_ontop(entity: GridEntity, _from_position: Vector3, new_pos: Vector3) -> bool:
	...
	entity.target_position = _from_position + self.basis.z.normalized() * entity.GRID_SIZE / 3
	await get_tree().create_timer(.2).timeout
